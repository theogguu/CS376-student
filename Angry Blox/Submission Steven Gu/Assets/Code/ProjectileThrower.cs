...
public class ProjectileThrower : MonoBehaviour {
    ...
    {
        foreach (var objects in FindObjectsOfType<Rigidbody2D>())
        {
            if (IsActive(objects) == true)
            {
                return true;
            }
        }
    ...
    {
        //Reloads if projectile has been fired and we aren't waiting for physics to settle 
        //or escape key is pressed.
        if ((firingState == FiringState.Firing) && (WaitingForPhysicsToSettle() == false) || 
